...
```text
data/raw/card_product/card_product_dummy.csv
...
& $PY .\scripts\eval\build_card_product_cases.py `
  --input .\data\raw\card_product\card_product_dummy.csv `
...
현재 repo에서는 `config/model_registry.yaml`에 실행 config를 등록하고, `config/eval_matrix.yaml` 또는 UI의 실행 탭에서 선택합니다.
...
현재 final_UI는 배포 판정과 Benchmark를 별도 영역으로 분리해 표시합니다. 실행 화면에서 `release_smoke`, `release_gate`, `benchmark_smoke`, `benchmark_full`, `custom_seeded_mix` profile을 선택하면 서버가 `config/eval_dataset_catalog.yaml`을 읽고 composer로 resolved JSONL을 만든 뒤 기존 runner에 `--cases-file`로 전달합니다.

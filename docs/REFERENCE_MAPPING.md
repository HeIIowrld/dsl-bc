...
| `KFinEval-Pilot.pdf` | 한국어 금융 평가에서는 한국어 표현, 국내 금융 업무 맥락, 출처별 분포가 중요함 | `docs/QUESTIONLIST_REGRESSION_WORKFLOW.md`, `data/QUESTION_SETS.md`, `scripts/eval/build_questionlist_cases.py`, `scripts/generate/generate_cases_from_domain_outputs.py` | BC카드/금융용어/FAQ/절차성 데이터 기반 한국어 테스트셋, 출처별 분포 요약 |

...
class EvalScoringTests(unittest.TestCase):
    ...
    def test_server_projection_keeps_reconciled_runs_sparse(self) -> None:
        handler = FinalUiHandler.__new__(FinalUiHandler)
        temp_parent = PROJECT_ROOT / "out"
        temp_parent.mkdir(exist_ok=True)
        with tempfile.TemporaryDirectory(dir=temp_parent) as tmp:
            ...
            run_dir = root / "RUN_RECONCILED"
            run_dir.mkdir()
            config = {
                "run_id": "RUN_RECONCILED",
                "run_type": "benchmark_dual_judge_arbiter_report",
                "case_source": str(cases_path),
            ...
            scores = [
                {
                    ...
                    "case_id": "C1",
                    "config_id": "model-a",
                    "llm_judge_config_id": "initial_judge_mean",
                    "llm_judge_status": "ok",
                    "llm_judge_acc": 20,
                    "llm_judge_com": 20,
                    "llm_judge_utl": 20,
...
class FinalUiJavascriptContractTests(unittest.TestCase):
    def test_benchmark_and_regression_tabs_are_separated_in_ui_contract(self) -> None:
        ...
        self.assertIn('const finalQaCategories = ["BC FAQ"', app_js)
        self.assertNotIn("\\uc0ac\\ub0b4FAQ", app_js)
        self.assertNotIn("사내FAQ", app_js)

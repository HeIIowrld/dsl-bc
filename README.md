...
├─ model/           Ollama 모델 생성용 Modelfile 예시
├─ schemas/         데이터 구조 검증용 JSON Schema
├─ scripts/         평가 실행, Judge 채점, 데이터 구성, 점검 스크립트
├─ sources/         원천 데이터 구성 관련 스크립트
...
| `sources/bc_cs_notice/corpus/` | 원천 문서 코퍼스를 직접 구성하는 경우의 입력 자료 |

대용량 원본 데이터, 실행 결과, API key, 개인 환경 파일은 로컬 환경에서 관리하는 것을 권장합니다. `.gitignore`는 폴더 구조는 유지하면서 폴더 안의 로컬 데이터 파일은 추적하지 않도록 구성되어 있습니다.
...
| `OLLAMA_BASE_URL` | Ollama 서버 주소. 기본값: `http://127.0.0.1:11434` |
...
이 플랫폼은 특정 모델 가중치나 평가 데이터를 내장하지 않습니다. 평가하려는 모델을 `config/model_registry.yaml` 또는 UI의 모델 등록 화면에 등록한 뒤 파이프라인에서 `--config` 값으로 선택해 실행합니다.
